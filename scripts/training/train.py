...
def main(args):
    ...
    # Only load external audio encoder if not resuming from checkpoint
    if not args.continue_unet_ckpt:
    ...
    for k in range(epo):
        for nm in range(0, len(files) + 1 - p, p):
            ...
            fmusic = model(os.path.join(args.train_audio_path, str(front0) + ".wav"), 16000)["embedding"]
            fcontent = load_image(os.path.join(args.train_image_path, str(front0) + ".jpg"), transform, shape=[256, 256])

            for ni in range(1,p):
                file_ext = os.path.splitext(files[nm+ni])
                front, ext = file_ext
                content = load_image(os.path.join(args.train_image_path, str(front) + ".jpg"), transform, shape=[256, 256])
                fcontent = torch.cat((fcontent, content), -4)

                music= model(os.path.join(args.train_audio_path, str(front) + ".wav"),16000)["embedding"]
