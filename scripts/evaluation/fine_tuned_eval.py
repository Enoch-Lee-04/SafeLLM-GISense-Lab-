...
def extract_risk_level(response):
    """Extract risk level (HIGH/MEDIUM/LOW) from response"""
    # Look for explicit "Overall Risk Level:" pattern
    pattern = r'Overall Risk Level:\s*(LOW|MEDIUM|HIGH)'
    match = re.search(pattern, response, re.IGNORECASE)
    if match:
        return match.group(1).upper()
    
    # Look for "Risk Level:" pattern
    pattern = r'Risk Level:\s*(LOW|MEDIUM|HIGH)'
    match = re.search(pattern, response, re.IGNORECASE)
    if match:
        return match.group(1).upper()
...
def evaluate_all_images(model, processor, image_folder, output_excel):
    ...
    for image_file in tqdm(image_files, desc="Evaluating images"):
        ...
        # Risk assessment
        risk_response, _ = generate_safety_assessment(
            model, processor, str(image_file), "risks"
        )
        ...
        all_results.append({
            ...
            'Risk Level': risk_level if risk_level else "N/A",
            'Risk Response': risk_response,
            'Response Length': len(response),
            'Has Score': "Yes" if score else "No",
            'Has Risk Level': "Yes" if risk_level else "No",
    ...
    with pd.ExcelWriter(output_excel, engine='openpyxl') as writer:
        ...
        stats_data = {
            ...
            'Value': [
                ...
                df['Has Risk Level'].value_counts().get('Yes', 0),
                df['Has Risk Level'].value_counts().get('No', 0),
    ...
    print(f"  - Successful risk levels: {df['Has Risk Level'].value_counts().get('Yes', 0)}")
    print(f"  - Failed risk levels: {df['Has Risk Level'].value_counts().get('No', 0)}")
